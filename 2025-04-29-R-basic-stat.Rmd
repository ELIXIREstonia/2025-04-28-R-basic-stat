...
Now let's run the ANOVA test (hint: `aov()`, ensure that left-hand is numeric, and right-hand is a factor) and take a look at the summary of the test (`summary()`).
...
the species explains almost all the variability in peal length
...
Theory: <https://zenodo.org/records/15294211/files/Corr_simple_reg_25_new_ver.pdf?download=1>
...
<https://zenodo.org/records/15282248/files/Corr_simple_reg_25_new_ver.pdf?download=1>Check the assumption for Pearson correlation:
...
Example scenario: Suppose you have a group of 100 students: 60 are left-handed, 40 are right-handed and 70 of them like cats and 30 like dogs. We want to test whether there is an association between handedness (right, left) and pet preference (dog, cat).

Let's create a contingency table:

```{r}
data <- matrix(c(60, 40, 30, 70), nrow = 2, byrow = TRUE)
rownames(data) <- c("Left-handed", "Right-handed")
